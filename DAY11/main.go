...
func (w *Wallet) AddMoney(amount float64) error {
	if amount <= 0 {
		return fmt.Errorf("AddMoney: %w", ErrInvalidAmount)
	}
	w.Balance += amount
...
func (w *Wallet) SendMoney(amount float64, receiver *Wallet) error {
	if amount <= 0 {
		return fmt.Errorf("SendMoney: %w", ErrInvalidAmount)
	}
	if w.Balance < amount {
		return fmt.Errorf("SendMoney: %w", ErrInsufficientBalance)
	}
	w.Balance -= amount
...
func CreateUser(name, email, phoneNumber string, initialBalance float64) (*User, error) {
	if initialBalance < 0 {
		return nil, fmt.Errorf("CreateUser: %w", ErrInvalidAmount)
	}
	return &User{
		Name:        name,
		Email:       email,
...
func main() {
	...
	for {
		...
		case "1":
			...
			newUser, err := CreateUser(name, email, phoneNumber, initialBalance)
			if err != nil {
		...
		case "3":
			...
			err = users[userChoice-1].Wallet.AddMoney(amount)
			if err != nil {
				fmt.Println("Error:", err)
			}
